...
public final class AMQPChannel {
    ...
    private let channel: AMQPChannelHandler<AMQPConnectionMultiplexHandler>
    ...
    public func addConsumeListener(consumerTag: String, listener: @escaping @Sendable (Result<AMQPResponse.Channel.Message.Delivery, Error>) -> Void) throws {
        return try self.channel.addConsumeListener(named: consumerTag, listener: listener)
    }

    /// Remove consume listener.
    /// - Parameters:
    ///     - consumerTag: name of consumer.
    public func removeConsumeListener(consumerTag: String) throws {
        return try self.channel.removeConsumeListener(named: consumerTag)
    }
    ...
    func removeListener<Value>(type: Value.Type, named name: String) throws {
        switch type {
            case is AMQPResponse.Channel.Message.Delivery.Type:
                return try removeConsumeListener(consumerTag: name)
